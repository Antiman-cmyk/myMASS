        ...
        with torch.no_grad():
            ...
            for data_set in ['valid', 'test']:
                ...
                for lang1, lang2 in set(params.mt_steps + [(l2, l3) for _, l2, l3 in params.bt_steps] + mass_steps):
                    ...
                    self.evaluate_mt_v2(scores, data_set, lang1, lang2, eval_bleu, trainer)
...
class EncDecEvaluator(Evaluator):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            if eval_bleu:
                ...
                if params.beam_size == 1:
                    generated, lengths = decoder.generate(enc1, len1, lang2_id, max_len=max_len, sample_temperature=params.temperature)
                else:
    ...
    # psl
    def evaluate_mt_v2(self, scores, data_set, lang1, lang2, eval_bleu, trainer):
        """
        Evaluate perplexity and next word prediction accuracy.
        """
        params = self.params
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            # dec2 = decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)
            # decode y3
            y3, len_y3 = decoder.generate(enc1, len1, lang2_id, max_len=int(1.5 * len1.max().item() + 10), sample_temperature=params.temperature)
            langs_y3 = y3.clone().fill_(lang2_id)

            # clm y3 -> y4
            # dec_y3 = decoder('fwd', x=y3, lengths=len_y3, langs=langs_y3, causal=True)
            # dec_y3 = dec_y3.transpose(0, 1)
            # dec_y4 = decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=dec_y3, src_len=len_y3)

            # mlm y3 -> y4
            y4, len_y4, langs_y4 = trainer.prompt_mlm(y3, len_y3, lang2_id, x1, lang1_id)
            # y4, len_y4, langs_y4 = trainer.prompt_xlmr(y3, len_y3, lang2_id, x1, len1, lang1_id)  # xlmr to predict mask word
            # y4, len_y4, langs_y4 = trainer.prompt_mlm(y3, len_y3, lang2_id, y3, lang2_id)

            # 1. 用enc_y4作为decoder的输入生成word scores
            # enc_y4 = encoder('fwd', x=y4, lengths=len_y4, langs=langs_y4, causal=False)
            # 2. 用dec_y4作为decoder的输入生成word scores
            # 2.1. src_enc = dec_y3 是一种
            # dec_y3 = decoder('fwd', x=y3, lengths=len_y3, langs=langs_y3, causal=True)
            # dec_y3 = dec_y3.transpose(0, 1)
            # dec_y4 = decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=dec_y3, src_len=len_y3)
            # 2.2 src_enc = enc1 是一种，感觉这种更合理
            # dec_y4 = decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=dec_y3, src_len=len_y3)
            dec_y4 = decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=len1)

            # loss
            # word_scores, loss = decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=True)
            word_scores, loss = decoder('predict', tensor=dec_y4, pred_mask=pred_mask, y=y, get_scores=True)
            # word_scores, loss = decoder('predict', tensor=enc_y4, pred_mask=pred_mask, y=y, get_scores=True)
            ...
            if eval_bleu:  
                ...
                if params.beam_size == 1:
                    # generated, lengths = decoder.generate(enc1, len1, lang2_id, max_len=max_len, sample_temperature=params.temperature)
                    generated, lengths = y3, len_y3
                    # y4, len_y4 = decoder.generate(dec_y3, len_y3, lang2_id, max_len=max_len, sample_temperature=params.temperature)
                    generated2, lengths2 = y4, len_y4
                else:
                    generated, lengths = decoder.generate_beam(
                        enc1, len1, lang2_id, beam_size=params.beam_size,
                        length_penalty=params.length_penalty,
                ...
                hypothesis.extend(convert_to_text(generated, lengths, self.dico, params))
                hypothesis2.extend(convert_to_text(generated2, lengths2, self.dico, params))
        ...
        if eval_bleu:
            ...
            # hypothesis / reference paths
            hyp_name = 'hyp{0}.{1}-{2}.{3}.y3.txt'.format(scores['epoch'], lang1, lang2, data_set)
            hyp_path = os.path.join(params.hyp_path, hyp_name)
            ref_path = params.ref_paths[(lang1, lang2, data_set)]
            hyp_name2 = 'hyp{0}.{1}-{2}.{3}.y4.txt'.format(scores['epoch'], lang1, lang2, data_set)
            hyp_path2 = os.path.join(params.hyp_path, hyp_name2)
            # ref_path2 = params.ref_paths[(lang1, lang2, data_set)]
            ...
            # evaluate BLEU score
            bleu = eval_moses_bleu(ref_path, hyp_path)
            bleu2 = eval_moses_bleu(ref_path, hyp_path2)
            logger.info("BLEU_y3 %s %s : %f" % (hyp_path, ref_path, bleu))
            logger.info("BLEU_y4 %s %s : %f" % (hyp_path2, ref_path, bleu2))
            scores['%s_%s-%s_mt_bleu_y3' % (data_set, lang1, lang2)] = bleu
            scores['%s_%s-%s_mt_bleu_y4' % (data_set, lang1, lang2)] = bleu2
...
def convert_to_text(batch, lengths, dico, params):
    ...
    # assert (batch[-1] == params.eos_index).sum() == bs
    sentences = []
