...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input, mask, kv=None, cache=None, segment_label=None):
        ...
        # psl
        if mask.size(-1) != klen:
            print('mask do not match mask_reshape: %s mask vs %s mask_reshape' % (mask.size(), mask_reshape))
        assert mask.size(-1) == klen, 'mask do not match mask_reshape: %s mask vs %s mask_reshape' % (mask.size(), mask_reshape)
        ...
        q = q / math.sqrt(dim_per_head)                                       # (bs, n_heads, qlen, dim_per_head)  scale
        scores = torch.matmul(q, k.transpose(2, 3))                           # (bs, n_heads, qlen, klen) matmul
        mask = (mask == 0).view(mask_reshape).expand_as(scores)               # (bs, n_heads, qlen, klen)
        scores.masked_fill_(mask, -float('inf'))                              # (bs, n_heads, qlen, klen)  mask

        weights = F.softmax(scores.float(), dim=-1).type_as(scores)           # (bs, n_heads, qlen, klen)  softmax
        weights = F.dropout(weights, p=self.dropout, training=self.training)  # (bs, n_heads, qlen, klen)
        context = torch.matmul(weights, v)                                    # (bs, n_heads, qlen, dim_per_head)  output
        context = unshape(context)                                            # (bs, qlen, dim)

        # multihead 最后再经过一层线形层映射到原维度
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, dico, is_encoder, with_output):
        ...
        # psl
        # softprompt
        # self.prompt_len = SOFT_PROMPT_LEN
        # self.softprompts = nn.ModuleList([nn.Linear(1, params.emb_dim) for i in range(self.prompt_len)])
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None, cache=None, enc_mask=None):
        ...
        if langs is not None:
            assert langs.size() == (slen, bs), 'mask do not match size: %s mask vs %s (slen,bs)' % (langs.size(), (slen, bs))
        ...
        #     for softprompt in self.softprompts:
        #         inputs_embeds = torch.cat((softprompt(src_enc).to(inputs_embeds.device)).unsqueeze(1) ,inputs_embeds),dim=1)
        #         # psl
        #         # attention_mask = F.pad(attention_mask, pad=(1, 0, 0, 0), mode='constant', value=True)
    ...
    def predict(self, tensor, pred_mask, y, get_scores):
        ...
        # psl
        return scores, loss
        # return masked_tensor, scores, loss
