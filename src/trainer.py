...
from .utils import get_optimizer, to_cuda, to_cpu, concat_batches
from .utils import parse_lambda_config, update_lambdas
from src.evaluation.evaluator import convert_to_text
from src.model import build_multiLM
...
class Trainer(object):
    ...
    def mask_out(self, x, lengths):
        ...
        if params.sample_alpha == 0:
            pred_mask = np.random.rand(slen, bs) <= params.word_pred  # 随机产生(slen,bs)的概率，小于params.word_pred的被mask
        ...
        # do not predict padding and first word
        ...
        _x_real = x[pred_mask]
        _x_rand = _x_real.clone().random_(params.n_words)  # 把真实的词随机替换词典中的词
        _x_mask = _x_real.clone().fill_(params.mask_index)  # mask token id in vocab
        probs = torch.multinomial(params.pred_probs, len(_x_real),
                                  replacement=True)  # 对input的每一行做n_samples次取值，输出的张量是每一次取值时input张量对应行的下标，replacement代表有无放回地取样  # 对每一个词概率性地选择 mask/keep/rand 0.8/0.1/0.1
        _x = _x_mask * (probs == 0).long() + _x_real * (probs == 1).long() + _x_rand * (
                    probs == 2).long()  # 把mask/keep/rand 拼回一个x
        x = x.masked_scatter(pred_mask, _x)  # pred_mask存的是batch里要mask/keep/rand的words（为1的），用_x去处理这些为1的words

        assert 0 <= x.min() <= x.max() < params.n_words
        assert x.size() == (slen, bs)
        assert pred_mask.size() == (slen, bs)

        return x, _x_real, pred_mask  # mask/keep/rand处理后的x, 被处理words的原来真实值, 处理words的 mask 标签

    # psl
    def mask_out_v2(self, x, lengths):  # mask word by word, let x be a sentence not a batch
        """
        words in turns to mask out, and target they get assigned.
        """
        params = self.params
        # slen, bs = x.size()
        slen, bs = x.size()
        ...
        pred_mask = alen[:, None] == alen[None]
        # pred_mask = torch.where(pred_mask, 1, 0)
        # if params.sample_alpha == 0:
        #     pred_mask = np.random.rand(slen, bs) <= params.word_pred  # 随机产生(slen,bs)的概率，小于params.word_pred的被mask
        #     pred_mask = torch.from_numpy(pred_mask.astype(np.uint8))
        # else:
        ...
        pred_mask[0] = False  # TODO: remove
        pred_mask[x == params.eos_index] = False  # do not predict eos
        ...
        cnt = 0
        ptr = 1
        # select_mask = torch.ones(pred_mask.size()).type(torch.bool)
        for i in range(len(_x_real)):
            id = _x_real[i]
            word = self.data['dico'].id2word[id.item()]
            if '@@' in word:
                cnt += 1
                continue
            elif cnt != 0:
                # select_mask[:, ptr+1:ptr+cnt+1] = False
                pred_mask[ptr:ptr + cnt + 1, ptr] = True
                pred_mask[:, ptr + 1:ptr + cnt + 1] = False
                ptr += cnt + 1
                cnt = 0
            else:
                ptr += 1
        # pred_mask = pred_mask[select_mask].view(pred_mask.size(dim=0), -1)
        ...
        _x_mask = _x_real.clone().fill_(params.mask_index)  # mask token id in vocab
        # psl
        # probs = torch.multinomial(params.pred_probs, len(_x_real), replacement=True).to(device=_x_real.device)  # 对input的每一行做n_samples次取值，输出的张量是每一次取值时input张量对应行的下标，replacement代表有无放回地取样  # 对每一个词概率性地选择 mask/keep/rand 0.8/0.1/0.1
        # _x = _x_mask * (probs == 0).long() + _x_real * (probs == 1).long() + _x_rand * (probs == 2).long()  # 把mask/keep/rand 拼回一个x
        _x = _x_mask.long()
        x = x.masked_scatter(pred_mask, _x)  # pred_mask存的是batch里要mask/keep/rand的words（为1的），用_x去处理这些为1的words
    ...
    def reload_checkpoint(self):
        ...
        if data['best_stopping_criterion']:
            self.best_stopping_criterion = data['best_stopping_criterion']
    ...
    def save_best_model(self, scores):
        ...
        for metric, biggest in self.metrics:
            ...
            if metric not in self.best_metrics:
                logger.warning(
                    "Metric \"%s\" not found in self.best_metric! Choose in %s!" % (metric, self.best_metrics))
                logger.info("Metrics in self.metric are %s" % self.metrics)
    ...
    def end_epoch(self, scores):
        ...
        if self.stopping_criterion is not None and (
            ...
            if factor * scores[metric] > factor * self.best_stopping_criterion:
                ...
                logger.info("Not a better validation score than %f (%i / %i)."
                            % (self.best_stopping_criterion, self.decrease_counts, self.decrease_counts_max))
    ...
    def clm_step(self, lang1, lang2, lambda_coeff):
        ...
        self.stats['processed_w'] += pred_mask.sum().itebtm()
...
class EncDecTrainer(Trainer):
    ...
    def __init__(self, encoder, decoder, data, params):
        ...
        if params.multiLM != '':
            xlmr_model, xlmr_tokenizer = build_multiLM(params)
            self.xlmr_model = xlmr_model
            self.xlmr_toenizer = xlmr_tokenizer

        if params.codes_path != '' and params.vocab_path != '':
            self.bpe = fastBPE.fastBPE(params.codes_path, params.vocab_path)
    ...
    def prompt_mlm(self, x2, len2, lang2_id, x1, lang1_id):
        """
        x2 as input, x1 as prompt
        x3 as mlm output
        """
        params = self.params

        for i in range(x2.size(dim=1)):
            x = x2[:, i, None].repeat(1, x2.size(dim=0))
            lengths = len2.max().repeat(x2.size(dim=0))
            # prompt_alen = torch.arange(prompt.size(dim=0), dtype=torch.long, device=lengths.device)
            # prompt_mask = prompt_alen[:, None] < -1
            x, y, pred_mask = self.mask_out_v2(x, lengths)
            langs = x.clone().fill_(lang2_id)
            positions = None
            # prompt
            # trans = torch.tensor(self.data['dico'].word2id['translate'])[None][None].to(device=x1.device)
            # prompt = torch.cat((x1[:, i, None], trans), dim=0).repeat(1, x2.size(dim=0))
            # # prompt = x1[:, i, None].repeat(1, x2.size(dim=0))
            # prompt_mask = torch.where(prompt.clone().fill_(False) == 0, False, True)
            # prompt_langs = prompt.clone().fill_(lang1_id)
            # x = torch.cat((prompt, x), dim=0)
            # pred_mask = torch.cat((prompt_mask, pred_mask), dim=0)
            # lengths = lengths.add_(prompt.size(dim=0))
            # langs = torch.cat((prompt_langs, langs), dim=0)

            if params.cuda:
                x, y, pred_mask, lengths, positions, langs = to_cuda(x, y, pred_mask, lengths, positions, langs)

            # forward / loss
            tensor = self.encoder('fwd', x=x, lengths=lengths, positions=positions, langs=langs, causal=False)
            scores, loss = self.encoder('predict', tensor=tensor, pred_mask=pred_mask, y=y, get_scores=True)
            # select next words: sample or greedy
            if params.temperature is None:
                # pred_words = torch.topk(scores, 1)[1].squeeze(1)  # topk=1
                pred_words = torch.topk(scores, params.topk)[1].squeeze(1)
            else:
                pred_words = torch.multinomial(F.softmax(scores / params.temperature, dim=1), 1).squeeze(1)
            # if convert_to_text
            eos = torch.tensor(self.data['dico'].eos_index)[None].to(device=pred_words.device)
            pred_words = torch.cat((eos, (torch.cat((pred_words, eos), dim=0))), dim=0)
            # len_words = torch.tensor(pred_words.size())
            pred_words = pred_words[:, None]
            pred_len = torch.tensor(pred_words.size(dim=0))[None].to(device=pred_words.device)
            # out = convert_to_text(next_words, len_words, self.data['dico'], params)
            # need to pad
            if i == 0:
                x3 = pred_words
                len3 = torch.tensor(pred_words.size(dim=0))[None].to(device=pred_words.device)
            else:
                if pred_len > len3.max():
                    pad = torch.LongTensor(pred_len - len3.max(), x3.size(1)).fill_(self.data['dico'].pad_index).to(
                        device=x3.device)
                    x3 = torch.cat((x3, pad), dim=0)
                elif pred_len < len3.max():
                    pad = torch.LongTensor(len3.max() - pred_len, pred_words.size(1)).fill_(
                        self.data['dico'].pad_index).to(device=pred_words.device)
                    pred_words = torch.cat((pred_words, pad), dim=0)
                x3 = torch.cat((x3, pred_words), dim=1)
                len3 = torch.cat((len3, torch.tensor(pred_words.size(dim=0))[None].to(device=len3.device)), dim=0)
        langs3 = x3.clone().fill_(lang2_id)
        return x3, len3, langs3

    def prompt_xlmr(self, x2, len2, lang2_id, x1, len1, lang1_id):
        """
        x2 as input, x1 as prompt
        x3 as mlm output using xlmr
        """
        params = self.params
        xlmr_model = self.xlmr_model
        xlmr_model.eval()
        tokenizer = self.xlmr_toenizer
        dico = self.data['dico']
        ...
        # save gpu memory
        print('xlmr to gpu, mass to cpu')
        self.encoder, self.decoder = to_cpu(self.encoder, self.decoder)
        self.xlmr_model = self.xlmr_model.to('cuda')
        torch.cuda.empty_cache()

        for i in range(x2.size(dim=1)):
            # orignal mass ids to string
            x = x2[:, i, None]
            length = len2.max()[None]
            txt = convert_to_text(x, length, dico, params)[0]
            txt = txt.replace('@@ ', '')
            prompt = x1[:, i, None]
            prompt_len = len1.max()[None]
            prompt_txt = convert_to_text(prompt, prompt_len, dico, params)[0]

            # string to tokens for xlmr
            tokens = tokenizer.tokenize(txt)
            prompt_tokens = tokenizer.tokenize(prompt_txt)

            # tokens to ids for xlmr
            ids = tokenizer.convert_tokens_to_ids(tokens)
            prompt_ids = tokenizer.convert_tokens_to_ids(prompt_tokens)

            # to tensor
            ids = torch.tensor([ids], device=x2.device)
            ids = ids.repeat(ids.size(dim=1), 1)
            # prompt_ids = torch.tensor([prompt_ids + [sep_id]])
            prompt_ids = torch.tensor([prompt_ids], device=x1.device)
            prompt_ids = prompt_ids.repeat(ids.size(dim=1), 1)

            # mask for select
            slen, bs = ids.size()
            alen = torch.arange(slen, dtype=torch.long, device=length.device)
            pred_mask = alen[:, None] == alen[None]
            prompt_mask = torch.where(prompt_ids.clone().fill_(False) == 0, False, True)

            # concat prompt and txt
            input_ids = torch.cat((prompt_ids, ids), dim=1)
            input_mask = torch.cat((prompt_mask, pred_mask), dim=1)
            _x_real = input_ids[input_mask]
            _x_mask = _x_real.clone().fill_(mask_id)
            _x = _x_mask.long()
            x = input_ids.masked_scatter(input_mask, _x)

            # predict
            # mask_index = torch.nonzero(x==mask_id)
            with torch.no_grad():
                outputs = xlmr_model(x).logits
            predict_ids = torch.argmax(outputs, dim=2)[input_mask]
            # predict_tokens = tokenizer.convert_ids_to_tokens(predict_ids)
            # predict_txt = tokenizer.convert_tokens_to_string(predict_tokens)
            predict_txt = tokenizer.decode(predict_ids, skip_special_tokens=True)
            # change bpe for mass
            # for j in range(len(predict_tokens)):
            #     if predict_tokens[j].startswith('▁'):
            #         predict_tokens[j] = predict_tokens[j].lstrip('▁') + '@@'
            # predict_ids = torch.tensor([self.data['dico'].word2id[j] for j in predict_tokens], device=predict_ids.device)

            # codes_path = r'/data/codes_ende'
            # vocab_path = r'/data/vocab_ende'
            predict_tokens = bpe.apply([predict_txt])
            predict_tokens = predict_tokens[0].split(' ')
            predict_ids = torch.tensor([dico.word2id[j] for j in predict_tokens if j in dico],
                                       device=predict_ids.device)  # TODO: word not in dic
            eos = torch.tensor(self.data['dico'].eos_index)[None].to(device=predict_ids.device)
            predict_ids = torch.cat((eos, (torch.cat((predict_ids, eos), dim=0))), dim=0)
            predict_ids = predict_ids[:, None]
            predict_len = torch.tensor(predict_ids.size(dim=0))[None].to(device=predict_ids.device)

            # to batch
            if i == 0:
                x3 = predict_ids
                len3 = torch.tensor(predict_ids.size(dim=0))[None].to(device=predict_ids.device)
            else:
                if predict_len > len3.max():
                    pad = torch.LongTensor(predict_len - len3.max()[None], x3.size(1)).fill_(
                        self.data['dico'].pad_index).to(device=x3.device)
                    x3 = torch.cat((x3, pad), dim=0)
                elif predict_len < len3.max():
                    # pad = torch.LongTensor(len3.max()[None] - predict_len, predict_len.size(1)).fill_(self.data['dico'].pad_index).to(device=predict_len.device)
                    pad = torch.LongTensor(len3.max()[None] - predict_len, predict_len.size(0)).fill_(
                        self.data['dico'].pad_index).to(device=predict_len.device)  # 一句话一句话处理 没有第二维度
                    predict_ids = torch.cat((predict_ids, pad), dim=0)
                x3 = torch.cat((x3, predict_ids), dim=1)
                len3 = torch.cat((len3, torch.tensor(predict_ids.size(dim=0))[None].to(device=len3.device)), dim=0)
        langs3 = x3.clone().fill_(lang2_id)

        print('xlmr to cpu, mass to gpu\n')
        self.xlmr_model = self.xlmr_model.to('cpu')
        self.encoder, self.decoder = to_cuda(self.encoder, self.decoder)
        torch.cuda.empty_cache()
    ...
    def bt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        with torch.no_grad():
            ...
            if params.prefix:
                # v1 x1,x2都是ids  直接相连
                # x2 = torch.cat((torch.tensor([32599]*x2.size(1))[None], x2), dim=0)
                # len2 = len1 + len2 + 1
                # x2 = torch.cat((x1, x2), dim=0)
                # langs2 =
                # x2 = torch.cat((x1, x2), dim=0)
                # len2 = len1 + len2
                # langs2 = torch.cat((langs1, langs2), dim=0)
                # del enc1

                # v2
                # alen = torch.arange(x2.shape[0], dtype=torch.long, device=len2.device)
                # pred_mask = alen[x1.shape[0]:, None] < len2[None] - 1
                # pred_mask[:len1] = 0
                # if params.context_size > 0:  # do not predict without context
                #     pred_mask[:params.context_size] = 0
                # y = x2[1:].masked_select(pred_mask[:-1])
                # assert pred_mask.sum().item() == y.size(0)
                # x2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, src_enc=enc1, src_len=len1, causal=True)
                ...
                # assert pred_mask.sum().item() == y.size(0)
                # x2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, src_enc=enc1, src_len=len1, causal=True)
                # # x2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True)
                # x2 = x2.transpose(0, 1)
                # x2, len2 = self.decoder.generate(x2, len2, lang2_id, max_len=int(1.3 * len1.max().item() + 5))
                # langs2 = x2.clone().fill_(lang2_id)

                # v4 mlm
                # x3, len3, langs3 = self.prompt_mlm(x2, len2, lang2_id, x1, lang1_id)
                x3, len3, langs3 = self.prompt_xlmr(x2, len2, lang2_id, x1, lang1_id)
                # x3, len3, langs3 = self.prompt_mlm(x2, len2, lang2_id, x2, lang2_id)  # x2 as prompt
        ...
        # enc2 = enc2.transpose(0, 1)
        # v4
        enc2 = self.encoder('fwd', x=x3, lengths=len3, langs=langs3, causal=False)
        enc2 = enc2.transpose(0, 1)

        # words to predict
        alen = torch.arange(len1.max(), dtype=torch.long, device=len1.device)
        pred_mask = alen[:, None] < len1[None] - 1  # do not predict anything given the last target word (len-1, bs)
        y1 = x1[1:].masked_select(pred_mask[:-1])  # 把x1[1:]之后的各行按照pred_mask连接成一行
        ...
        try:
            # dec3 = self.decoder('fwd', x=x1, lengths=len1, langs=langs1, causal=True, src_enc=enc2, src_len=len2)
            dec3 = self.decoder('fwd', x=x1, lengths=len1, langs=langs1, causal=True, src_enc=enc2, src_len=len3)
        except:
            return
        # loss
